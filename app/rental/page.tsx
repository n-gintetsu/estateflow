...
export default function RentalProperties() {
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      equipment: form.equipment ? (Array.isArray(form.equipment) ? form.equipment : form.equipment.split('、').map((s: string) => s.trim()).filter(Boolean)) : null,
        ...
        <div style={{ background: 'white', borderRadius: 12, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', overflow: 'hidden' }}>
          {loading ? (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              visibility: i.visibility || 'both' }); setUploadPreviews(Array.isArray(i.images) ? i.images : []); setShowForm(true) }} style={{ background: '#e8f4fd', color: '#1a3a5c', border: '1px solid #b3d4f0', padding: '6px 12px', borderRadius: 6, cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>
                      ✏️ 編集
                    </button>
